...
class Utils:
    ...
    def generate_intermediate_samples(generator, global_i, session, writer=None, collateImages = True):
        ...
        for outer_count in range(samplesRepeatN):
            ...
            images = []
            for _ in range(rowN):              
                myz = utils.conditional_to_cuda(Variable(torch.randn(args.n_label * colN, args.nz)))
            ...
            if collateImages:
                ...
                if writer:
                    writer.add_images('samples_latest_{}'.format(session.phase), torch.cat(images, 0), session.phase)
    ...
    def reconstruct_images(generator, encoder, loader, global_i, nr_of_imgs, prefix, reals, reconstructions, session, writer=None): #of the form"/[dir]"
        ...
        if reconstructions and nr_of_imgs > 0:
            ...
            if writer:
                writer.add_images('reconstruction_latest_{}'.format(session.phase), t[:nr_of_imgs] / 2 + 0.5, session.phase)
                # Igor: changed add_image to add_images
    ...
    def interpolate_images(generator, encoder, loader, epoch, prefix, session, writer=None):
        ...
        if True:
            ...
            real_image, _ = next(dataset)
            Utils.interpolation_set_x = utils.conditional_to_cuda(Variable(real_image, volatile=True))
        ...
        if writer:
            writer.add_images('interpolation_latest_{}'.format(session.phase), t / 2 + 0.5, session.phase)
            # Igor: changed add_image to add_images
