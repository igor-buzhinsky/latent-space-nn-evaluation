...
class Session:
    def __init__(self):
        ...
        self.generator = nn.DataParallel(utils.conditional_to_cuda(Generator(args.nz+1, args.n_label)))
        self.g_running = nn.DataParallel(utils.conditional_to_cuda(Generator(args.nz+1, args.n_label)))
        self.encoder   = nn.DataParallel(utils.conditional_to_cuda(Discriminator(nz = args.nz+1, n_label = args.n_label, binary_predictor = args.train_mode == config.MODE_GAN)))
    ...
    def load(self, path):
        checkpoint = utils.conditional_torch_load(path)
...
def accumulate(model1, model2, decay=0.999):
    ...
    for k in par1.keys():
        par1[k].mul_(decay).add_(1 - decay, par2[k].data)

def get_grad_penalty(discriminator, real_image, fake_image, step, alpha):
    """ Used in WGAN-GP version only. """
    eps = utils.conditional_to_cuda(torch.rand(batch_size_by_phase(step), 1, 1, 1))
...
def D_prediction_of_G_output(generator, encoder, step, alpha):
    ...
    myz = utils.conditional_to_cuda(Variable(torch.randn(batch_size_by_phase(step), args.nz)), args.gpu_count > 1)
...
def train(generator, encoder, g_running, train_data_loader, test_data_loader, session, total_steps, train_mode):
    ...
    while session.sample_i < total_steps:
        ...
        z = utils.conditional_to_cuda(Variable( torch.FloatTensor(batch_size(reso), args.nz, 1, 1) ), args.gpu_count > 1)
        ...
        if train_mode == config.MODE_GAN:
            ...
            if args.use_loss_fake_D_KL:
                ...
                if m > 0.0 and session.phase >= 5:
                    KL_loss = torch.min(torch.ones_like(KL_fake) * m / 2,
            ...
            if len(e_losses) > 0:
                e_loss = sum(e_losses)                
                stats['E_loss'] = e_loss.detach().cpu().float().numpy()#np.float32(e_loss.data)
                e_loss.backward()
        ...
        if (batch_count + 1) % args.n_critic == 0:
            ...
            for _ in range(args.n_generator):
                ...
                if len(g_losses) > 0:
                    loss = sum(g_losses)
                    stats['G_loss'] = loss.detach().cpu().float().numpy()#np.float32(loss.data)
                    loss.backward()
                ...
                if train_mode == config.MODE_CYCLIC:
                    if args.use_loss_z_reco:
                        stats['z_reconstruction_error'] = z_diff #FIXME .data[0]
