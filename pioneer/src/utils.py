...
def conditional_to_cuda(x: torch.tensor, non_blocking: bool = False) -> torch.tensor:
...
def conditional_torch_load(path: str) -> torch.tensor:
    return torch.load(path) if args.gpu_count > 0 else torch.load(path, map_location=torch.device("cpu"))
